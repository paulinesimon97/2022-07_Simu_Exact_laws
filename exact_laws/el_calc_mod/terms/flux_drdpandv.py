...
class FluxDrdpandv(AbstractTerm):
    ...
    def calc(self, vector:List[int], cube_size:List[int], rho, pperp, ppar, vx, vy, vz, pm, bx, by, bz, **kwarg) -> List[float]:
        return calc_flux_with_numba(calc_in_point, *vector, *cube_size, rho, pperp, ppar, vx, vy, vz, pm, bx, by, bz)
...
@njit
def calc_in_point(i, j, k, ip, jp, kp, rho, pperp, ppar, vx, vy, vz, pm, bx, by, bz):
